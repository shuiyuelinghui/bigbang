...
module.exports={
    ...
    "plugins":{
       ...
       "jqgrid":"jqgrid/jquery.jqGrid.min.js",
        ...
        "bootstrap":"bootstrap/bootstrap.3.31.js",
...
window.sysParams=window.sysParams||localData.get('sysParam')||{};
...
var ajInfoAction = makeAct('qualityCheck/cflrzpxc/aj_info',0);


if(typeof module === "object" && typeof module.exports === "object" ){
    module.exports={
        //查看案件信息
        ajInfoCheckByAjJqBh :function(ajbh, jqbh) {
            if(ajbh == null && jqbh == null) return false;
            $post(ajInfoAction,{ajbh:ajbh,slBjslh:jqbh},function(res){
                var data = res.data;
                if(data == null) {
                    toast("查不到数据！",600).warn();
                    return false;
                };
                top.registry.global.ajxxData = data;
                var checkWin=$open('quality-check-ajinfo.html', {width: 820, title: '案件基本信息',
                    onClose:function(){
                        typeof window.ajInfoCheckCloseFn=='function' && ajInfoCheckCloseFn();
                    }},true,function (){
                    intoAjInfoCheck(checkWin);
                });
            },true)
        },
        //打开word控件页面
...
* */
    ...
    window.extending({config:config},true);
...
},{"../data/config.json":1,"./business/pub-business":2,"./lib/defineding.js":4,"./lib/eui":5,"./lib/exy":6,"./lib/jquery":9,"./lib/jquery.cookie":7,"./lib/locals":11,"./lib/mock-register.js":12,"./lib/paging.js":13,"./lib/patch.js":14,"./lib/path.js":15,"./lib/proto":17,"./lib/pub":18,"./lib/stp":23,"./lib/validate.js":25}],4:[function(require,module,exports){
/**
...
module.exports={
        ...
        //是否出现滚动条 也可以设置overflow-y:hidden,overflow:visible,再恢复保存的overflow值,来比较两次是否有变化来判断
        ...
        $loadHTML:function(src,cb){
            if(window.loadHTMLCache[src]){
                ...
                $.get(getViewPath(src),function(res){
...
var prefix='@prjName:'+window.config.prjName+'-';//+config.version+'-'
var localData={};
localData.extending({
    set:function(key,val){
        key=prefix+key;
...
(function(){
    ...
    var mockActionsArr=[
        ...
        'sys/param',
        'dict/gxsdm',
        'dict/search/commonDict/root.AJLBDM',
        'system/xtdh/list',
        'system/message/list',
        'app/myapps/allList',
        'jsgl/notic/view/init',
        'jsgl/notic/view/queryfst',
        'jsgl/technician/personInfo',
        'stat/sbtj/echarts',
        'stat/sbtj/kyzl/echarts',
        'workbench/jqxx/list',
        'workbench/jqxx/count',
        'workbench/hjtjbd/list',
        'workbench/assign/list',
        'workbench/sceneInfo/list',
        'workSupervise/ajjskyjd/list',
        'alims/wt/query',
        'alims/wt/echarts',
        'alims/DNA/query',
        'evidence/application/in/query',
        'evidence/application/out/query',
        'evidence/application/chart',
        'evidence/application/query',
        'qualityCheck/dqp/list',
        'qualityCheck/kybl/list',
        'qualityCheck/xczp/list',
        'qualityCheck/ajkycc/list',
        'qualityCheck/cflrxc/list',
        'qualityCheck/bhgxcjc/list',
        'qualityCheck/cflrzpxc/list',
        'qualityCheck/cflrzpxc/list',
        'stat/dqpSceneInvest/list',
        'stat/sbtj/list',
        'jsgl/technician/list',
        'jsgl/technician/rp/1/list',
        'jsgl/technician/rp/2/list',
        'jsgl/technician/gzll/list',
        'jsgl/technician/kycg/list',
        'jsgl/technician/qual/1/list',
        'jsgl/technician/qual/2/list',
        'jsgl/technician/xxqk/list',
        'jsgl/technician/lwzzqk/list'
    ];
    ...
    //for(var n in  mockActions){
    //    mockActions[n]=basePath+ (mockActions[n]||window['camel2dash'](n)) +'.json';
    //}
    window.config=window.config||{};
    window.config.actions=window.config.actions||{};
    window.config.mock && (window.config.mockActions=mockActions);
})();

//可以在base最下方再手动注册
// config.actions['qualityCheck/cflrxc/list']=makeAct('api/1/qualityCheck/cflrxc/list');
// config.actions['qualityCheck/cflrxc/list']=makeAct('qualityCheck/cflrxc/list',1);
 ...
 (function($){
	...
	//paging是利用pagination的封装。 在含有paging分页条的母容器上发动。总数count作为option的一部分传入，而callback从option中拿出单独传入。
 	$.fn.paging = function(config,clickHandle) {
			...
			var paceOnceSetingHTML='<div class="select-pageonce"><label>每页显示</label><select name="datatable_length"><option value="15">15</option><option value="30">30</option><option value="50">50</option><option value="100">100</option></select>条</div>'
		    ...
		    var opts = {
				...
				num_display_entries:config.num_display_entries||6,
			...
			//显示每页几条自定义
			if(config.count>pageOnce && $this.children('.paging').children('.select-pageonce').length==0){
		...
		//pagingList只接收一个对象做参数
		 $.fn.pagingList=function(config) {
			...
			var remote=function(cb,reset,_begin,_end){
				...
				//mark 分页更改,设置缓存为整页的倍数, 且起点为缓存的倍数
				if(useCache){
					jsonObj['pageSize'] = cacheOnce;
				...
				if(config.loading!==false){
					config.beforeSend=showLoading;
                    config.complete=hideLoading;
				}
				//拓展支持全部$.ajax配置
				//$[method](action, $.extend({jsonStr: obj2str(jsonObj)}, params))
				$.ajax($.extend({
					url:action,
					type:method,
					data: $.extend({jsonStr: obj2str(jsonObj)}, params)
				},config||null)).always(function (res, status) {
					if(config.beforeSend=showLoading){
                        hideLoading();
					}
			...
			if(config.count){
			 ...
			 var sortFn=function(config,ct){
				 ...
				 if(sortOrder=='desc'){
					 ...
					 ct.find('[sort-name={0}]'.format(sortName)).addClass('no-init-sort-icon').append('<b class="sort-arrow fr gray">▼</b>');
				 }else{
...
(function(){
    ...
    //     var yjmenu =new gui.Menu({ type: 'menubar' });
    //     gui.Window.get().menu = yjmenu;
    //     yjmenu.append(new gui.MenuItem({ label: ' 复 制 ' }));
    //     _win.menu = yjmenu;
    //     yjmenu.items[0].click = function() {
    //         document.execCommand('copy');
    //     };
    //     document.body.addEventListener('contextmenu', function (e) {
    //         e.preventDefault();
    //         var x=e.x+($('body').hasClass('sb-l-m')?60:200);
    //         yjmenu.popup(window==top?e.x:x, window==top?e.y:e.y+90);
    //         return false;
    //     });
    //     //});
    // }
...
window.token=window.token||localData.get('token')||'';
...
window.getViewPath=function (src,prefix){
    ...
    if(prefix){
        viewPrePath=prefix+src;
    }else if(config.isLocal){
        viewPrePath=getDistPath()+'/view/';
    }else{
        viewPrePath= location.pathname.match(/\/index\.html|\/login\.html|\\index\.html|\\login\.html/) ? './view/' : './';
    }
    var querySymbol=viewPrePath.indexOf('?')>-1?'&':'?';
    return viewPrePath+(src||'404.html')+  querySymbol +'version='+window.config.version;
};
window.getMap_server= function () { return  window.defaultMapServer ? window.defaultMapServer:(window.defaultMapServer=localData.get('sysParam')['defaultMapServer']);};//window.config.mapServerPath;};
...
window.$.fn.previewBox=function(orgSrcAttr,eveType,hasWrap){
    ...
    previewWrap=previewWrap.length?previewWrap:$('<div class="preview-wrap"><img/><a class="icon-remove"></a></div>').appendTo('body');
    var previewImg=previewWrap.children('img');

    this.each(function(){
        var $this=$(this);
        //info('reg： '+$this.attr('preview-reged'))
...
},{"./lambda":10}],18:[function(require,module,exports){
...
module.exports={
    ...
    $open:function(str,params,isAjax,cb){
        ...
        (params.width=='max' || params.width>window.innerWidth) && (params.width=window.innerWidth-40);
        ...
        if(params.mask=='global'){
            ...
            params.top=params.top||(useFullWrap?50:30);
        ...
        maxobj2=useFullWrap?{visibility:'visible',maxHeight:maxHeight-36-fixBarHeight}:{visibility:'visible',maxWidth:maxWidth-20,maxHeight:maxHeight-36-fixBarHeight};//36+48=84,36为弹窗自带的title条的高度,48为底部固定按钮的高度
        ...
        if(ele || str.indexOf('#')==0 ){
            ...
            if(window.loadHTMLCache[str]){
                ...
                $.ajax({
                    ...
                    ele.window(params).css(maxobj2).html(res).parent().addClass('animated fadeInDown').css(maxobj).end();
                    cb&&cb();
                    fixBtn(ele);
                });
            }
    ...
    $ajax:function(url,params,cb,jsonStrWrap,type,beforeSend,complete){
        var ag=arguments[0];
        if(typeof ag=='object'){
            url=ag.url, params=ag.data||ag.params, cb=ag.success||ag.callback, jsonStrWrap=ag.jsonStrWrap, type=ag.type||type, beforeSend=ag.beforeSend, complete=ag.complete;
        }
    ...
    $post:function(url,params,cb,jsonStrWrap,beforeSend,complete){
        return $ajax(url,params,cb,jsonStrWrap,'POST',beforeSend,complete);
    },
    $get:function(url,params,cb,jsonStrWrap,beforeSend,complete){
        return $ajax(url,params,cb,jsonStrWrap,'GET',beforeSend,complete);
    },
    ...
    makeAct:function(act,log){
        ...
        var actKey=act.replace(/^\//,'').replace(/\?.+/,'');

        if(window.config.mock){
            if(act.indexOf('dict/search/commonDict')>-1){
                actKey=actKey.replace('api/','')+ '/'+queryParse(act).root;
            }
            return acts[actKey]||window.getDistPath()+'mock/'+act.replace(/\//g,'-')+'.json';
        }else{
            var url;
            switch (log){
                case 1:url='/api/1/'+act;   break;
                case 0:url='/api/0/'+act;   break;
                case '':url='/api/'+act;   break;
                default:url='/'+act;    break;
            }
            return acts[actKey]||(window.path+url);
        }
    },
    /**
...
},{"./jquery.extend":8,"./previewbox":16}],19:[function(require,module,exports){
/**
 * Created by yao on 2016/11/15.
 */
//自定义列
var initCustomCol = function (customArr,wraper,cb) {
    var customObj = {};
    var $wrap = $(wraper||document.body);
    // $wrap.data('customCols-checkedArr',[]);
    var i = 0,
        cusArrLen = customArr.length,
        cusI = {},
        localPI = '',
        localP = top.localParams.global,
        jsonObj = {},
        customColNameArr = customArr.select('o=>o.en'),
        customColNameArrLen = customColNameArr.length,
        tableName,
        $tb = $wrap.is('table') ? $wrap : $wrap.find('table').first();

    var createCustomCol = function (customArr) {
        var i = 0,
            $customColBlock = $('<div id="{0}-custom-col-block" class="{0}-custom-col-block hide"><ul class="{0}-custom-col-required"></ul><ul class="{0}-custom-col-free"></ul><div class="custom-col-btn-wrap"><b class="cm-save-btn custom-col-confirm-btn"></b><b class="cm-cancel-btn custom-col-cancel-btn"></b></div></div>'.format(tableName)),//<ul class="{0}-custom-col-operate"></ul>
            $customColRequired = $customColBlock.find('.'+tableName+'-custom-col-required'),
            $customColFree= $customColBlock.find('.'+tableName+'-custom-col-free'),
            customArrLen = customArr.length,
            customArrI,
            liHtml,
            checkAllHtml = '<li><input type="checkbox" id="{0}-check-all"><label for="{0}-check-all"></label><em>全 选</em><label for="{0}-check-all" class="custom-col-switch" data-on="显示" data-off="隐藏"></label></li>'.format(tableName);
        // $customCol.append('<li><input type="checkbox" id="check-all"><label for="check-all">全 选</label></li>');
        for(;i<customArrLen;i++){
            customArrI = customArr[i];
            liHtml = '<li><input type="checkbox" id="{2}-check-{0}" {3} {4}><label for="{2}-check-{0}"></label><em>{1}</em><label for="{2}-check-{0}" class="custom-col-switch" data-on="显示" data-off="隐藏"></label></li>'.format(customArrI.en,customArrI.ch,tableName);
            if(customArrI.required){
                $customColRequired.append(liHtml.replace('{3}','disabled').replace('{4}','checked'));
            }else{
                $customColFree.append(liHtml);
            }
        }
        $customColFree.append(checkAllHtml);
        $customColBlock.appendTo('body');
    };
    var loadDataByCustomCol = function (customUl) {
        // var upCheckedArr=[];
        customUl.find('li').find('input[type="checkbox"]').each(function (i,el) {
            var customName = el.id.split('-')[2]+'Custom';
            if(el.checked){
                jsonObj[customName] = 1;
                // i!=0 && upCheckedArr.push(i-1);//except the checkbox for check-all
            }else{
                jsonObj[customName] = 0;
            }
            customObj[customName] = jsonObj[customName];
        });
        // var checkedArr=$wrap.data('customCols-checkedArr');
        // if(upCheckedArr.join()==checkedArr.join()){
        //     log('no changed');
        //     return true;
        // }
        localP.set(tableName,obj2str(jsonObj));
        //warn('checkedArr: '+checkedArr.join()),info('theUpdated: '+upCheckedArr.join()),info('changed!');
        cb(customObj,true);
        //createCustomCol(customArr);
    };
    var bindClick = function () {
        // $(document).on('click',function (e) {
        //     var $customUl = $('.custom-col');
        //     //点击空白处隐藏
        //     $(e.target).is('#custom-col-btn,.custom-col,a,span,b,input') || $customUl.has(e.target).length || $customUl.hasClass('hide')
        //     || loadDataByCustomCol($customUl.addClass('hide'));
        // });
        var $customColBlock = $('#{0}-custom-col-block'.format(tableName));
        var $customColFree = $('.{0}-custom-col-free'.format(tableName));
        var inputNotCheckAll = 'input[type="checkbox"][id!="{0}-check-all"]'.format(tableName);
        var checkAll = '{0}-check-all'.format(tableName);
        //展示或隐藏自定义列
        $(document).off('click','#custom-col-btn').on('click','#custom-col-btn',function () {
            var i = 0,
                customColNameArrI = {};
            //reset it before push the checked item
            // var checkedArr=[];
            var inputSelector = '';
            for(;i<customColNameArrLen;i++){
                customColNameArrI = customColNameArr[i];
                inputSelector = '#{0}-check-{1}'.format(tableName,customColNameArrI);
                if(jsonObj[customColNameArrI+'Custom']){
                    $(inputSelector).prop('checked',true);
                    // checkedArr.push(i);
                }else if(jsonObj[customColNameArrI+'Custom']===''){
                    $(inputSelector).prop('checked',false);
                }
            }
            // $wrap.data('customCols-checkedArr',checkedArr);
            //info('设置完成'+checkedArr.join())
            if($customColBlock.find('li').find('input:checked').not('#'+checkAll).length === customColNameArrLen){
                $('#'+checkAll).prop('checked',true);
            }else{
                $('#'+checkAll).prop('checked',false);
            }
            // }
            $open('#'+tableName+'-custom-col-block',{width:350,title:'自定义列'});
        }).off('click','#{0}-check-all'.format(tableName)).on('click','#{0}-check-all'.format(tableName),function () {//全选自定义列
            var checkInput = $customColFree.find('li').find('input[type="checkbox"]');
            if(this.checked){
        ...
        $('[id$="custom-col-block"]').off('click','li').on('click','li',function (e) {
            $(e.target).is('input,label') || $(this).find('input')[0].click();
        }).off('click','.custom-col-confirm-btn').on('click','.custom-col-confirm-btn',function () {
            loadDataByCustomCol($customColFree);
            $customColBlock.$close();
        }).off('click','.custom-col-cancel-btn').on('click','.custom-col-cancel-btn',function () {
            $customColBlock.$close();
            //全部选上时全选自动选中
        }).off('click',inputNotCheckAll).on('click',inputNotCheckAll,function () {
            var checkedAll = true;
            $customColFree.find(inputNotCheckAll).each(function (i,el) {
                checkedAll = checkedAll && el.checked;
            });
            checkedAll ? (byid(checkAll).checked = true) : (byid(checkAll).checked = false);
        });
    };

    if($tb.attr('name')){
        tableName = $tb.attr('name')
    }else{
        location.pathname.match(/.*-(\w+).html/);
        tableName = RegExp.$1;
    }

    jsonObj = str2obj(localP.get(tableName));
    jsonObj = jsonObj ? jsonObj : {};
    for(;i<cusArrLen;i++){
        cusI = customColNameArr[i]+'Custom';
        localPI = jsonObj[cusI];
        customObj[cusI] = localPI === undefined ? 0 : localPI;
    }
    cb(customObj);
    $('.{0}-custom-col-block'.format(tableName)).length || createCustomCol(customArr);
    bindClick();
    return customObj;
};
$.fn.customCol=function(customArr,cb){
    return this.data( 'customObj', initCustomCol(customArr||this.data('customArr'),this.closest('.query-result')[0],cb) );
};
typeof module === "object" && typeof module.exports === "object"  && (module.exports=initCustomCol);
...
var reg=/\<\/?[^\<\>]+\>/g;
//隐藏指定列的方法
var custtomHide=function(rs,table){
    var ths=table.children('thead').children('tr').children('th');
    table.hide();
    //debugger;
    ths.each(function(i){
        var act=rs[i].custom==0 ?  'addClass':'removeClass';
        var tds=table.children('tbody').children('tr').children('td:nth-child({0})'.format(i+1));

        $(this).add(tds)[act](hideClass);
    });
    table.show();
};
//小控件模版
var btnHtml='<div class="custom-tool"><i class="fa icon-cogs"></i></div>';
var winHtml='<div id="{0}" class="custom-setting-modal">'+
    ...
    '</div>';
var tp='<li><input type="checkbox" id="{1}"/><label for="{1}"></label><u>{0}</u><label for="{1}" class="custom-col-switch" data-on="显示" data-off="隐藏"></label></li>';//checked={1} readOnly={2}

$.fn.customCol=function(attr){
    var table=this;
    var attr=attr||'custom';//指令属性也可以自定义
    var key=(config.prjName||'')+(window.iframe?window.iframe.getAttribute('page-no')||'':'')+(table.prop('id')||'');
    var html2txt=function(html){return html.replace(reg,'').trim();};
...
//console.info(['取出的rs',rs])
    ...
    if(rs){
        if(rs.length!=ths.length){
            ...
            var needUp=rs.some(function(r,i){
                return r.cn!=html2txt(ths[i].innerHTML) || (r.custom==null && ths[i].hasAttribute(attr)) || (r.custom!=null && !ths[i].hasAttribute(attr));
            });
            needUp && (rs=null);
        }
    }
    if(!rs){
        ...
        ths.each(function(i){
            ...
            rs[i]={cn:html2txt(this.innerHTML),custom:this.hasAttribute(attr)?1:null};
        });
        localData.set(key,rs);
    }

    custtomHide(rs,table);

    //生成小控件
    var btn=$(btnHtml);
    var wrap=table.closest('.query-result');
    wrap.attr('custom-btn-appended') ||btn.appendTo(wrap.attr('custom-btn-appended',true));//设定为table必须有.query-result包裹，因为小控件不生成于table中
    ...
    btn.on('click',function(){
        ...
        rs.each(function(r,i){
            ops+=tp.format(r.cn,key+'-'+i);//,r.custom!=0,r.custom==null);
        });
        ...
        $open('#'+tempID,{width:350,title:'自定义列'});//$open接受元素传参
        ...
        win.on('click','.cm-save-btn',function(){
            ...
            .on('click','.cm-cancel-btn',function(){
                win.$close().remove(); //因为没有走单例，因此每次动态生成，使用完后都清除
...
//然后自启动
$(function(){
    config.autoCustomCol && window!=top && $('.query-result table').eq(0).customCol('cs');//:has("th[custom]:first")
});
...
window.$filter('sub10', function(){
    return this.trim().slice(0,10);
    //return (new Date(this)).format('YYYY-MM-DD');
},String);

window.$filter('asCnTime',function(){
    return (new Date(this)).format('YYYY年M月D日 hh:mm');
});
...
window.$.fn.fixTable=function fixTable(mode,cb){//,wraped,cb){
    ...
    setTimeout(function(){
        ...
        if( (mode=='head' || mode=='all') && needFixHead){
            ...
            wrapHeight=wrapHeight>245?wrapHeight:(window.height-100);
            ...
            var scrollWidth=hasScroll(nativeWrap[0],'y');
            //var scrollHeight=hasScroll(nativeWrap[0],'x');
            var rootWrapWidth=getComputedStyle(rootWrap[0]).width;
            //theCopyForHead.wrap('<div class="head-fix-wrap" style="width:{0};">'.format(rootWrapWidth)).width(Math.max(getComputedStyle(nativeTable[0]).width,rootWrapWidth));
            theCopyForHead.wrap('<div class="head-fix-wrap">').width(Math.max(getComputedStyle(nativeTable[0]).width,rootWrapWidth));
            theCopyForHead.parent().css('margin-right',scrollWidth).appendTo(rootWrap);
            //theCopyForHead.parent().css('width','calc(100% - {0}px)'.format(scrollWidth)).appendTo(rootWrap);

            //nativeTable的td居然会在computedStyle计算完成后, 再次自动扩展大小, 见于列少屏幕大的情况. 因此这种情况下, 重新拷贝
            //needFixCol=needFixCol && !scrollHeight ;
            //info([nativeTable[0].offsetWidth,nativeTable[0].parentNode.offsetWidth, needFixCol ,  needFixColEnd,parseInt(getComputedStyle(theCopyForHead[0]).width),parseInt(getComputedStyle(nativeTable[0]).width)])
            //由于复制的th取来的原型computedStyle.width变化,导致theCopyForHead宽度不适配
            ...
            if(theCopyForHeadWidth<nativeTableWidth-2 || theCopyForHeadWidth>nativeTableWidth+2){ //允许2px的误差
                log('need replace clone-head agian  scrollWidth:'+scrollWidth);
                theCopyForHead.css({width:'100%',margin:'0'}).parent().css({width:'100%','padding-right':hasScroll(nativeTable.parent()[0],'y'),background:'transparent'});

                //theCopyForHead.width(parseInt(getComputedStyle(nativeTable[0]).width)).children('thead').replaceWith(nativeTable.children('thead').clone());
    ...
    parent.document.addEventListener('webkitfullscreenchange',function(){
        setTimeout(function(){nativeTable.fixTable('all');},100)
    });
    return nativeTable;
};
...
var $template=(function($){
    ...
    return function (container,data,arg2,arg3){
        ...
        var initIt=function(){
            ...
            //$container.is('tbody') && $container.parent('table').fixCol().fixHead(true);
            if($container.is('tbody')){
               $container.parent('table').customCol('cs').fixTable('all');
                //allFixWrap=parentTable.closest('.all-fix-wrap');
                //if(allFixWrap.length>0){
                //    allFixWrap.replaceWith(parentTable);
                //    //parentTable.children('thead').children('tr').children('th').children('.custom-col-wrap').html(function(i,old){return old.replace(backUpWrapBegin,'').replace(backUpWrapEnd,'');});
                //}
               //table.fixTable('all');//,parentTable.parent().hasClass('native-fix-wrap'),dataLen);
            }else if($container.is('table')){
                $container.customCol('cs').fixTable('all');
            }
            return $container.data('current-data',data);
        };
        if(cache[tpsource]){
            //注意init抽取为function后,其代码中默认this指向变化,由于$(selector).template(...)会传入元素的fixData作为this,因此需要一路传进子方法,或者外层用the保存
            return initIt.call(this);
        }else if(tpsource.match(/\.html\s*$/)){
...
window.$.fn.template=function(data,arg2,arg3){
    return $template.apply(this.data('fix-data')||window,[this,data,arg2,arg3])//[this].concat([].slice.call(arguments));
};
window.$.fn.tpsource=function(tps){
    if(typeof tps=='undefined'){
          return this[0] ? this[0].getAttribute('tpsource') : null;
    }else{
      return this.each(function(){
          this.setAttribute('tpsource',typeof tps=='function'?tps(this):tps);
      });
  }
};
...
var initCustomCol=require('./stp.customcol');//old version
require('./stp.customcols');
var stp={
    $encode:$encode,
    $compile:$compile,
    $template:$template,
    initCustomCol:initCustomCol
}
//window.extending(obj);
typeof module === "object" && typeof module.exports === "object"  && (module.exports=stp);
},{"./stp.customcol":19,"./stp.customcols":20,"./stp.filter":21,"./stp.fixtable":22,"./stp.table":24}],24:[function(require,module,exports){
/**
 * Created by yao on 2016/11/15.
 */
$.fn.table=function(config){
    ...
    if(the.is('table')){
        //其实这里直接传了html字符串进template, 所以template内部将不再编译,也用不到helper和allowHTML, 但是为了将生成这个html的data作为第四项传入, 需要helper和allowHTML占位
        table=the.addClass('typical-tb stp-table '+ (config.cls||'')).template(html,helper,allowHTML,data).data('table-config',config);
    }else{
